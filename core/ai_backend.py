...
class GroqKeyPool:
    ...
    def __init__(self, keys: list):
        ...
        self._exhausted: dict = {}        # {key: recovery_deadline_timestamp} (parsed from error or default)
        ...
        self._current_idx = 0
        self._default_recovery_ttl = 3600  # Fallback if recovery time not parsed from error
        ...
        with self._lock:
            ...
            for idx, key in enumerate(self._keys):
                ...
                if key in self._exhausted:
                    ...
                    if now >= recovery_deadline:
                        ...
                        log.info(f"Key ...{key[-4:]} recovered (was: {reason})")
            ...
            if queryable:
                for i in range(len(queryable)):
                    ...
                    self._current_idx = (idx_in_queryable + 1) % max(len(queryable), 1)
                    return key
            
            # 2. All QUERYABLE keys used -> try the key that will recover soonest
            if recoverable:
                recoverable.sort(key=lambda x: x[0])  # Sort by recovery_deadline (soonest first)
                recovery_deadline, key = recoverable[0]
                soonest_recovery = max(0, recovery_deadline - now)
                log.debug(f"All QUERYABLE keys busy. Next key ...{key[-4:]} recovers in {soonest_recovery:.0f}s")
                # Still return it; caller will handle retry logic
                return key
        
        return None

    def mark_exhausted(self, key: str, reason: str = "Daily limit hit", error_str: str = None):
        ...
        with self._lock:
            if key not in self._exhausted:
                ...
                if error_str:
                    ...
                    log.debug(f"Parsed recovery window: {recovery_seconds:.0f}s from error message")
                else:
                    # Fallback to default recovery time
                    recovery_deadline = time.time() + self._default_recovery_ttl
                    recovery_seconds = self._default_recovery_ttl
                
                self._exhausted[key] = recovery_deadline  # Store deadline (not just timestamp)
    ...
    def mark_rpm_limited(self, key: str):
        ...
        with self._lock:
            if key not in self._exhausted:
                ...
                masked = key[:8] + "..." + key[-4:]
                log.info(f"Groq key {masked} temporarily skipped (RPM). Will retry after rotation.")

    def clear_rpm_flags(self):
        """Clear all RPM skip flags - call after trying all keys once."""
        with self._lock:
            if self._rpm_skipped:
                log.debug(f"Clearing RPM flags for {len(self._rpm_skipped)} key(s). Attempting retry.")
...
class AIBackend:
    def __init__(self, preferred_backend: str = None):
        ...
        if GROQ_API_KEY_POOL:
            ...
            if GROQ_FALLBACK_MODEL and GROQ_FALLBACK_MODEL == GROQ_MODEL:
                log.debug(f"GROQ_FALLBACK_MODEL == GROQ_MODEL (both '{GROQ_MODEL}'). Fallback disabled.")
            elif not GROQ_FALLBACK_MODEL:
                log.debug("GROQ_FALLBACK_MODEL is empty. Fallback to smaller model disabled.")
        ...
        log.debug(f"AI Backend active: {active} | {self._groq_pool.pool_status}")
    ...
    def _compact_prompt_text(self, text: str, max_chars: int, label: str = "prompt") -> str:
    ...
    def _prompt_budget_for_backend(self, backend: str, model_id: str = None) -> int:
    ...
    def _select_backend_order_for_prompt(self, prompt_chars: int) -> list:
        ...
        Smart escalation rules (in priority order):
          1. If Groq pool is fully exhausted -> skip Groq entirely, lead with Google/OpenRouter
          2. For very large prompts (>=50K chars) -> prefer Google then OpenRouter (bigger context)
          3. For medium prompts (>=25K) -> prefer OpenRouter then Groq
          4. Otherwise -> follow self.backend preference, then availability order
        """
        groq_alive = self._groq_pool.has_keys and not self._backend_is_cooled_down("groq")

        if not groq_alive:
            # Groq dead - immediately lead with free cloud backends
            preferred = (AI_NAME_GEMINI.lower(), AI_NAME_OPENROUTER.lower(), AI_NAME_OLLAMA.lower())
        elif prompt_chars >= 50000:
            # Large prompt - use big-context backends first to avoid truncation
            preferred = (AI_NAME_GEMINI.lower(), AI_NAME_OPENROUTER.lower(), AI_NAME_GROQ.lower(), AI_NAME_OLLAMA.lower())
        elif prompt_chars >= 25000:
            # Medium prompt - prefer OpenRouter (more generous limits than Groq for sustained load)
            preferred = (AI_NAME_OPENROUTER.lower(), AI_NAME_GROQ.lower(), AI_NAME_GEMINI.lower(), AI_NAME_OLLAMA.lower())
        else:
            preferred = ()
        ...
        for backend in (AI_NAME_GROQ.lower(), AI_NAME_OPENROUTER.lower(), AI_NAME_GEMINI.lower(), AI_NAME_OLLAMA.lower()):
            if backend not in ordered:
                ordered.append(backend)
    ...
    def _prepare_prompt_payload(self, backend: str, system_prompt: str, user_message: str, model_id: str = None) -> tuple[str, str, int]:
        ...
        compact_system = self._compact_prompt_text(system_prompt, system_cap, label=f"{backend} SYSTEM")
        compact_user = self._compact_prompt_text(user_message, user_cap, label=f"{backend} USER")
        log.warning(
    ...
    def _is_endpoint_reachable(self, url: str, timeout: int = 4) -> bool:
        """Lightweight connection check using browser User-Agent and streamed GET request."""
        try:
            headers = {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
            }
            # Streamed GET fetches only headers, extremely fast and compatible
    ...
    def _detect_available(self) -> list:
        ...
        # Priority 2: OpenRouter (fast cloud fallback when Groq is exhausted)
        if OPENROUTER_API_KEY:
            if not OPENROUTER_MODEL:
                log.debug(
                    "OpenRouter: API key found but OPENROUTER_MODEL is empty in .env "
                    "- backend will be skipped until configured."
                )
            else:
                available.append(AI_NAME_OPENROUTER.lower())
                if self._is_endpoint_reachable("https://openrouter.ai/api/v1/models"):
                    log.debug("OpenRouter API key found + endpoint reachable - openrouter fallback enabled.")
                else:
                    log.warning(
                        "[SYS.WARN] OpenRouter: API key configured but preflight check failed (timeout/DNS). "
                        "Fallback capability enabled but may experience execution stalls."
                    )
        else:
            log.debug("OpenRouter: skipped (no OPENROUTER_API_KEY in .env)")

        # Priority 3: Google Gemini
        if GOOGLE_API_KEY:
            available.append(AI_NAME_GEMINI.lower())
            if self._is_endpoint_reachable("https://generativelanguage.googleapis.com"):
                log.debug("Google Gemini API key found + endpoint reachable - free cloud fallback enabled.")
            else:
                log.warning(
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if any(OLLAMA_MODEL.split(":")[0] in m for m in models):
                    ...
                    log.debug(f"[!] Ollama: running but model {OLLAMA_MODEL} not found")
        except Exception:
            log.debug(f"[!] Ollama: skipped (connection refused at {OLLAMA_BASE_URL})")
    ...
    def check_all_backends(self) -> dict:
        ...
        report = {
            ...
            "openrouter": {"status": "OFFLINE", "configured": bool(OPENROUTER_API_KEY and OPENROUTER_MODEL), "details": ""}
        }
        ...
        if GROQ_API_KEY_POOL:
            for key in GROQ_API_KEY_POOL:
                ...
                try:
                    ...
                    self._query_backend("groq", "test", "hi", api_key=key)
                    report["groq"]["keys"].append({"key": masked, "status": "QUERYABLE"})
                except Exception as e:
                    report["groq"]["keys"].append({"key": masked, "status": "OFFLINE", "error": str(e)})
        ...
        if GOOGLE_API_KEY:
            try:
                # Use model metadata endpoint to verify key + model availability where possible.
                ...
                if r.status_code == 200:
                    ...
                    # Fall back to attempting a small generate call to capture detailed errors
                    try:
        ...
        if OPENROUTER_API_KEY and OPENROUTER_MODEL:
            try:
                if not OPENROUTER_MODEL:
                    report["openrouter"]["details"] = "OPENROUTER_MODEL is empty"
                else:
                    # quick models list to verify key works
                    headers = {"Authorization": f"Bearer {OPENROUTER_API_KEY}"}
                    r = requests.get("https://openrouter.ai/api/v1/models", headers=headers, timeout=5)
                    if r.status_code == 200:
                        report["openrouter"]["status"] = "ONLINE"
                    else:
                        report["openrouter"]["details"] = f"HTTP {r.status_code}: {r.text[:200]}"
            except Exception as e:
                report["openrouter"]["details"] = str(e)
        elif OPENROUTER_API_KEY and not OPENROUTER_MODEL:
            report["openrouter"]["details"] = "OPENROUTER_MODEL is empty"
    ...
    def get_backend_report(self) -> dict:
        ...
        # Groq status
        if GROQ_API_KEY_POOL:
            active_keys = len(self._groq_pool._keys) - len(self._groq_pool._exhausted)
        ...
        if OPENROUTER_API_KEY:
            if OPENROUTER_MODEL:
                report["backends"]["openrouter"] = {
                    "status": "ONLINE",
                    "detail": f"model: {OPENROUTER_MODEL}",
                    "icon": "[+]"
                }
            else:
                report["backends"]["openrouter"] = {
                    "status": "MISCONFIGURED",
                    "detail": "OPENROUTER_MODEL empty in .env",
                    "icon": "[!]"
                }
        else:
            report["backends"]["openrouter"] = {
                "status": "NOT_CONFIGURED",
                "detail": "no OPENROUTER_API_KEY in .env",
                "icon": "[x]"
            }
    ...
    def ai_available(self) -> bool:
        ...
        """
        if self._groq_pool.has_keys and not self._backend_is_cooled_down("groq"):
            return True
        if OPENROUTER_API_KEY and OPENROUTER_MODEL and not self._backend_is_cooled_down("openrouter"):
            return True
        if GOOGLE_API_KEY and not self._backend_is_cooled_down("google"):
            return True
        if "ollama" in self._available_backends and not self._backend_is_cooled_down("ollama"):
            return True
        return False

    def query(self, system_prompt: str, user_message: str, max_retries: int = 4, model_id: str = None) -> str:
        """
        ...
        # Reduce retry pressure on large prompts so we do not burn tokens on repeated retries.
        ...
        # This is deterministic and avoids another AI call just to shorten context.
        primary_backend = self.backend if self.backend in self._available_backends else None
        if primary_backend is None:
            primary_backend = "groq" if self._groq_pool.has_keys else (self._available_backends[0] if self._available_backends else "groq")
        system_prompt, user_message, _ = self._prepare_prompt_payload(primary_backend, system_prompt, user_message, model_id=model_id)
        ...
        for backend in backends_to_try:
            ...
            # Respect temporary backend cooldowns after repeated 429s / provider backpressure
            ...
            if backend == "groq":
                ...
                if groq_model is None:
                    # Prefer the smaller Groq fallback model for large prompts to reduce TPD pressure.
                    if prompt_chars >= 25000 and GROQ_FALLBACK_MODEL and GROQ_FALLBACK_MODEL != GROQ_MODEL:
                        groq_model = GROQ_FALLBACK_MODEL
                    else:
                        groq_model = GROQ_MODEL

                groq_result = self._query_groq_with_rotation(system_prompt, user_message, max_retries, model_id=groq_model)
                if groq_result is not None:
                    ...
                    if result_str:
                        ...
                        log.warning("[FIX 3.4] Groq returned empty response - treating as failure")
                        last_error = "Empty response from Groq"
                # All Groq keys failed -> fall through to next backend
                if not self._groq_pool.has_keys:
                    log.warning("[->] All Groq keys exhausted (TPD). Falling back to OpenRouter/Google/Ollama...")
                else:
                    log.warning("[->] Groq queries failed. Falling back to next backend...")
                continue
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    result = self._query_backend(backend, system_prompt, user_message, model_id=model_id)
                    # FIX #3.4: Validate result is not empty
                    if result and result.strip():
                        return result.strip()
                    elif result is not None:
                        log.warning(f"[FIX 3.4] {backend} returned empty response - treating as failure")
                        last_error = f"Empty response from {backend}"
                except Exception as e:
                    err_str = str(e).lower()
                    last_error = e

                    # Connection / DNS error: skip backend entirely to avoid NameResolutionError stalls
                    if "nameresolutionerror" in err_str or "gaierror" in err_str or "connection refused" in err_str:
                        log.error(f"[->] '{backend}' connection/DNS failure. Skipping to next backend.")
                        self._cooldown_backend(backend, 60, "connection failure")
                        break

                    # Quota / billing error: skip backend entirely
                    if "quota" in err_str or "billing" in err_str:
                        log.error(f"[->] '{backend}' quota/billing error. Skipping to next backend.")
                        break

                    # Rate limit: one short backoff, then advance to the next backend.
                    if "429" in err_str or "too many requests" in err_str or "rate_limit" in err_str:
                        wait = min(RATE_LIMIT_INITIAL_BACKOFF, 10)
                        self._cooldown_backend(backend, wait, "rate limit")
                        log.warning(f"[->] '{backend}' rate-limited (429). Skipping to next backend after cooldown.")
                        if attempt == 0:
    ...
    def _query_groq_with_rotation(self, system: str, user: str, max_retries: int, model_id: str = None) -> str | None:
        """
        ...
        for attempt in range(max_retries * max(len(self._groq_pool._keys), 1) + max_retries):
            # Get next key (skip RPM-flagged ones first; if none available, allow them)
            ...
            try:
                log.debug(f"Groq query with key ...{key[-4:]} (attempt {attempt + 1})")
                result = self._groq(system, user, api_key=key, model_id=model_id)
                if result and result.strip():
                    return result
            except Exception as e:
                err_str = str(e).lower()
                ...
                if self._is_tpd_error(e):
                    ...
                    if not tpd_fallback_tried and GROQ_FALLBACK_MODEL and GROQ_FALLBACK_MODEL != GROQ_MODEL:
                        ...
                        if next_key and next_key != key:
                            try:
                                result = self._groq(system, user, api_key=next_key, model_id=GROQ_FALLBACK_MODEL)
                                if result and result.strip():
                                    log.info(f"[+] Fallback model succeeded on key ...{next_key[-4:]}.")
                                    return result
                ...
                if is_auth_error:
                    self._groq_pool.mark_exhausted(key, reason="Invalid/Blocked key (auth error)")
                    if not self._groq_pool.has_keys:
                        log.error("All Groq keys are invalid/blocked (auth error).")
                        return None
                    continue
                ...
                if is_rpm:
                    ...
                    next_key = self._groq_pool.get_active_key(skip_rpm=True)
                    if next_key and next_key != key:
                        log.info(f"Groq RPM limit hit on ...{key[-4:]}. Rotating to ...{next_key[-4:]}.")
                        continue
                    else:
                        log.warning(f"Groq RPM limit hit and no other keys. Waiting {AI_RETRY_DELAY}s...")
                        self._cooldown_backend("groq", AI_RETRY_DELAY, "RPM limit")
                        self._groq_pool.clear_rpm_flags()
                ...
                delay = min(2 ** (attempt + 1), 20)
                log.warning(f"Groq attempt {attempt+1} failed: {e}. Retry in {delay}s...")
                time.sleep(delay)

                # After a few generic failures, try next key
                if attempt > 0 and attempt % 2 == 0:
                    self._groq_pool.mark_rpm_limited(key)  # Temporarily skip this key
    ...
    def _query_backend(self, backend: str, system: str, user: str, api_key: str = None, model_id: str = None) -> str:
        backend_lower = backend.lower()
        if backend_lower == AI_NAME_OLLAMA.lower():
            return self._ollama(system, user, model_id=model_id)
        elif backend_lower == AI_NAME_GROQ.lower():
            return self._groq(system, user, api_key=api_key, model_id=model_id)
        elif backend_lower in (AI_NAME_GEMINI.lower(), "google"):
            return self._google(system, user, model_id=model_id)
        elif backend_lower == AI_NAME_OPENROUTER.lower():
            return self._openrouter(system, user, model_id=model_id)
    ...
    def _is_tpd_error(self, exception: Exception) -> bool:
        ...
        if hasattr(exception, 'response') and isinstance(exception.response, dict):
            ...
            if error_code in ('daily_token_limit_exceeded', 'tokens_per_day_limit_exceeded'):
                return True
    ...
    def _groq(self, system: str, user: str, api_key: str = None, model_id: str = None) -> str:
        ...
        # Groq limit increased to preserve command context (Groq supports up to 128K with Pro)
        MAX_CHARS = 100000
        ...
        # Let exceptions (including TPD) bubble up naturally for key rotation to handle
    ...
    def _google_api_version(self, model: str = None) -> str:
        ...
        # gemini-2.x and gemini-1.5-x require v1beta
        if model_lower.startswith("gemini-2.") or "2.5" in model_lower or "2.0" in model_lower:
    ...
    def _google(self, system: str, user: str, model_id: str = None) -> str:
        ...
        model = model_id or GOOGLE_MODEL
        api_version = self._google_api_version(model)
        url = (
            f"https://generativelanguage.googleapis.com/{api_version}/models/"
            f"{model}:generateContent?key={GOOGLE_API_KEY}"
        )
        log.debug(f"Google Gemini: using API version {api_version} for model {model}")
        ...
        if r.status_code == 404:
            raise ValueError(
                f"Gemini 404 - model '{GOOGLE_MODEL}' not found on {api_version} endpoint. "
                f"Check GOOGLE_MODEL in .env (e.g. gemini-1.5-flash or gemini-2.0-flash)"
            )
        r.raise_for_status()
        data = r.json()
        ...
        if prompt_feedback.get("blockReason"):
            raise ValueError(f"Gemini blocked prompt: {prompt_feedback.get('blockReason')}")
        ...
        if finish_reason in ("SAFETY", "RECITATION", "OTHER"):
            raise ValueError(f"Gemini refused response (finishReason={finish_reason})")
    ...
    def _openrouter(self, system: str, user: str, model_id: str = None) -> str:
        """Query OpenRouter API (https://openrouter.ai/api)."""
        ...
        r = requests.post(url, json=payload, headers=headers, timeout=TOOL_AI_TIMEOUT)
        r.raise_for_status()
        data = r.json()
        # Support both choices/message and direct content shapes
        if "choices" in data and data["choices"]:
            choice = data["choices"][0]
            if "message" in choice and "content" in choice["message"]:
                return choice["message"]["content"]
            if "message" in choice and "text" in choice["message"]:
                return choice["message"]["text"]
        # Fallback: try 'result' or 'output' keys
        if "result" in data and isinstance(data["result"], str):
            return data["result"]
        if "output" in data and isinstance(data["output"], str):
            return data["output"]
        raise ValueError(f"OpenRouter returned unexpected response: {data}")
