...
def banner():
    ...
    if width >= 90:
        # High-impact massive 3D custom block logo for large terminals (72 chars wide)
        logo_lines = [
            "  █████  ██   ██  █████   █████  ███████ ██   ██ ██████  ██████  ███████ ",
            " ██   ██ ██   ██ ██   ██ ██        ██    ██   ██   ██    ██   ██ ██      ",
            " ██   ██ ███████ ██   ██  █████    ██    ██ █ ██   ██    ██████  █████   ",
        ...
        gradient_colors = ["#00f5d4", "#06b6d4", "#3b82f6", "#8b5cf6", "#d946ef", "#f43f5e"]
    else:
        ...
        gradient_colors = ["#00f5d4", "#06b6d4", "#3b82f6", "#8b5cf6", "#f43f5e"]
    ...
    grid.add_row(
        ...
        Text.from_markup(f"[bold {C_BRIGHT}]Multi-Agent Reasoning & Incremental Learning[/]")
    )
    grid.add_row(
        Text.from_markup(f"[bold {C_ACCENT2}]● STEALTH[/]"),
        Text.from_markup(f"[bold {C_BRIGHT}]WAF Ghost Proxy Chains & IP Rotation Matrix[/]")
    )
    grid.add_row(
        Text.from_markup(f"[bold {C_SUCCESS}]● DATABASE[/]"),
        Text.from_markup(f"[bold {C_BRIGHT}]Actor-Model Transaction sqlite3 WAL Database[/]")
    )
    grid.add_row(
        Text.from_markup(f"[bold {C_DANGER}]● EXECUTION[/]"),
        Text.from_markup(f"[bold {C_BRIGHT}]Isolated Payload Sandbox & Remote VPS Environment[/]")
    )
...
def section(title: str):
    ...
    rule = f"[bold {C_ACCENT}]{sec_text}[/]" + f"[bold {C_DIM}]" + "━" * remaining + "[/]"
...
def preflight_check(label: str, status: str, ok: bool):
    ...
    plain_len = len(label) + (len(status) + 3 if status and status != "NOMINAL" and status != "Windows (Degraded)" else 0)
...
def mission_briefing(params: dict):
    ...
    _add_row("Cognitive Engine ", params.get("ai", "OLLAMA").upper(), C_ACCENT2)
    _add_row("Assessment Mode  ", params.get("mode", "PENTEST").upper(), C_ACCENT)
    _add_row("Execution Node   ", params.get("execution", "LOCAL").upper(), C_BRIGHT)
    ...
    _add_row("Threat Intel     ", ", ".join(os_items) if os_items else "PASSIVE", C_SUCCESS if os_items else C_DIM)
    
    st_items = params.get("stealth_list", [])
    _add_row("Evasion Policy   ", ", ".join(st_items) if st_items else "DEGRADED", C_SUCCESS if st_items else C_DANGER)
...
def kill_chain_status(all_phases: list[str], current_phase: str = None, completed: dict = None):
...
def phase_header(phase_name: str, description: str = ""):
    ...
    remaining = max(10, term_width - len(sec_text) - 4)
    rule = f"[bold {p['c']}]{sec_text}[/]" + f"[bold {C_DIM}]" + "━" * remaining + "[/]"
    console.print(Text.from_markup(rule))
    if description:
        console.print(Text.from_markup(f"  [bold {C_DIM}]Scope: {description}[/]"))
    console.print()

def phase_complete(phase_name: str, status: str, duration_s: float, findings_count: int = 0):
    p = PHASES.get(phase_name, {"c": C_ACCENT, "tag": phase_name.upper()[:3]})
    ...
    console.print(Text.from_markup(f"  [bold {C_ACCENT}]◆ Phase {p['tag']} complete[/]  Status: {st}  |  Duration: {dur}{findings_str}"))
    console.print()

@contextlib.contextmanager
def phase_spinner(phase_name: str):
    p = PHASES.get(phase_name, {"c": C_ACCENT, "tag": phase_name.upper()[:3]})
    label = f"  [bold {p['c']}]◆ {p['tag']}[/] [bold white]Active ReAct Strategy Reasoning Execution...[/bold white]"
...
def engagement_dashboard(phases: dict, total_duration_s: float, engagement_id: str, target: str):
    t_min, t_sec = divmod(int(total_duration_s), 60)
    t_hr, t_min = divmod(t_min, 60)
    total_str = f"{t_hr}h {t_min}m {t_sec}s" if t_hr > 0 else (f"{t_min}m {t_sec}s" if t_min > 0 else f"{t_sec}s")
    ...
    info_table.add_row("  ❯ Vulns Logged    ", f"[bold {C_SUCCESS}]{total_findings}[/bold {C_SUCCESS}]")
